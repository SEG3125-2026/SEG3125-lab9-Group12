...
function BrowsePage() {
  ...
  const { t } = useLanguage()
  const { decks, recentDecks, deleteDeck, isLoading, loadError, recordDeckAccess, restoreDeck } =
    useDeckLibrary()
  ...
  return (
    <div className="page">
      ...
      <section className="surface-panel">
        <div className="section-heading">
          ...
          <span className="badge">
            {filteredDecks.length} {t('browse.shown')}
          </span>
        </div>

        {isLoading ? (
          <div className="empty-state">
            <h3>Loading deck library</h3>
            <p>Reading decks from the local SQL database.</p>
          </div>
        ) : loadError ? (
          <div className="empty-state">
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          await handleDelete(deckToDelete.id)
